...
class disALEXI(object):
    ...
    * :func:`calc_T_S_series`. Soil temperature from soil sensible heat flux and resistance in series.
    ...
    def DisALEXI_PT(self,
        ...
        e_atm = 1.0 - (0.2811 * (np.exp(
            -0.0003523 * ((T_A_Kresize - 273.16) ** 2))))  # atmospheric emissivity (clear-sly) Idso and Jackson (1969)
    ...
    def runDisALEXI(self, xStart, yStart, ALEXIgeodict, TSEB_only):
        ...
        if TSEB_only == 1:
            ...
            if not os.path.exists(outFN):
                ...
                ls.clone(mask, ta)
                subprocess.check_output('gdal_fillnodata.py %s %s -mask %s -of GTiff' % (outfile, masked, mask),
                                        shell=True)
        ...
        difvis, difnir, fvis, fnir = calc_difuse_ratio(Rs_1, sZ,
                                                       press=p)  # fraction of difuse and PAR/NIR radiation from shortwave irradiance (W m-2, solar zenith angle, atmospheric pressure and precipitable water vapour )
        ...
        if TSEB_only == 1:
            ...
            e_atm = 1.0 - (0.2811 * (np.exp(
                -0.0003523 * ((T_A_K - 273.16) ** 2))))  # atmospheric emissivity (clear-sly) Idso and Jackson (1969)
